...
func ParseVersionNegotiationPacket(b *bytes.Reader) (*Header, []quic.VersionNumber, error) {
	...
	versions := make([]quic.VersionNumber, b.Len()/4)
	for i := 0; b.Len() > 0; i++ {
		v, err := utils.BigEndian.ReadUint32(b)
		if err != nil {
			return nil, nil, err
		}
		versions[i] = quic.VersionNumber(v)
	}
	return hdr, versions, nil
}

// ComposeVersionNegotiation composes a Version Negotiation
func ComposeVersionNegotiation(destConnID, srcConnID protocol.ConnectionID, versions []quic.VersionNumber) []byte {
