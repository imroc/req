...
func parseDatagramFrame(r *bytes.Reader, _ quic.VersionNumber) (*DatagramFrame, error) {
...
func (f *DatagramFrame) Write(b *bytes.Buffer, _ quic.VersionNumber) error {
	typeByte := uint8(0x30)
	if f.DataLenPresent {
		typeByte ^= 0x1
	}
...
func (f *DatagramFrame) MaxDataLen(maxSize protocol.ByteCount, version quic.VersionNumber) protocol.ByteCount {
...
func (f *DatagramFrame) Length(_ quic.VersionNumber) protocol.ByteCount {
