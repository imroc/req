...
func parseConnectionCloseFrame(r *bytes.Reader, _ quic.VersionNumber) (*ConnectionCloseFrame, error) {
...
func (f *ConnectionCloseFrame) Length(quic.VersionNumber) protocol.ByteCount {
...
func (f *ConnectionCloseFrame) Write(b *bytes.Buffer, version quic.VersionNumber) error {
	if f.IsApplicationError {
		b.WriteByte(0x1d)
	} else {
		b.WriteByte(0x1c)
