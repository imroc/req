...
var _ = Describe("Initial AEAD using AES-GCM", func() {
	...
	DescribeTable("computes the client key and IV",
		func(v quic.VersionNumber, expectedClientSecret, expectedKey, expectedIV []byte) {
	...
	DescribeTable("computes the server key and IV",
		func(v quic.VersionNumber, expectedServerSecret, expectedKey, expectedIV []byte) {
	...
	DescribeTable("encrypts the client's Initial",
		func(v quic.VersionNumber, header, data, expectedSample []byte, expectedHdrFirstByte byte, expectedHdr, expectedPacket []byte) {
	...
	DescribeTable("encrypts the server's Initial",
		func(v quic.VersionNumber, header, data, expectedSample, expectedHdr, expectedPacket []byte) {
	...
	for _, ver := range []quic.VersionNumber{protocol.VersionDraft29, protocol.Version1, protocol.Version2} {
		v := ver
